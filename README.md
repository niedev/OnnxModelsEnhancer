...
This guide is for an Ubuntu enviroment, if you use Windows you can try to replicate the logic with equivalent windows command, ore use WSL.
...
In that case, two copies of the decoder are created: one that generates the kv-cache on the first iteration, and the other (with_past) that has the kv-cache among its inputs and is used for subsequent iterations.

My version, however, uses a custom architecture, that have a single decoder copy, and a small model dedicated to the initial kv-cache creation. Plus, since encoder and decoder in Madlad have the same embedding matrix, the embedding is separated in a dedicated model, to avoid the embedding matrix duplication.

With this tecniques, the non quantized model drops its size from 
